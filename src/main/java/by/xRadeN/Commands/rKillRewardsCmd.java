...
public class rKillRewardsCmd implements CommandExecutor {
    ...
    private boolean prefixEnabled() { return rKillRewards.getInstance().getConfig().getBoolean("Settings.prefix-enabled");}
    ...
    public boolean onCommand(CommandSender sender, Command command, String lable, String[] args) {
        if(sender.hasPermission("rkillrewards.reload") || sender.isOp()) {
            switch (args.length < 1 ? 0 :
                ...
                case 0:
                case 2: {
                    if(prefixEnabled()) sender.sendMessage(this.translate("Messages.prefix") + this.translate("Messages.usage"));
                ...
                case 1: {
                    if(args[0].equalsIgnoreCase("reload")) {
                        ...
                        if(prefixEnabled()) sender.sendMessage(this.translate("Messages.prefix") + this.translate("Messages.reload-config-message"));
                        else sender.sendMessage(this.translate("Messages.reload-config-message"));
                    } else if(prefixEnabled()) sender.sendMessage(this.translate("Messages.prefix") + this.translate("Messages.usage"));
                    else sender.sendMessage(this.translate("Messages.usage"));
                    break;
                }
            }
        } else if(prefixEnabled()) sender.sendMessage(this.translate("Messages.prefix") + this.translate("Messages.no-permission-message"));
